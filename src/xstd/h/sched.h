...
#ifndef POLYGRAPH__XSTD_H_SCHED_H
#define POLYGRAPH__XSTD_H_SCHED_H
...
inline int CpuCount(const cpu_set_t *set) {
    int count = 0;
...
// CPU_AND replacement
inline void CpuAnd(cpu_set_t *destset, const cpu_set_t *srcset1, const cpu_set_t *srcset2) {
    CPU_ZERO(destset);
    for (int i = 0; i < CPU_SETSIZE; ++i) {
...
// TODO: Consider wrapping cpuset_setid() and friends on FreeBSD.
...
#define CPU_EQUAL(set1, set2) 0
#define CPU_SET(ind, set) (void)0
#define CPU_CLR(ind, set) (void)0
#define CPU_ISSET(ind, set) 0
...
#endif /* POLYGRAPH__XSTD_H_SCHED_H */
